...
/* Commit subjects are quoted in the index; the lane's copy has no em dashes. */
function plain(s) { return String(s || '').replace(/\s*\u2014\s*/g, ': '); }
...
list.forEach((c, i) => {
  ...
  const format = { commit: c.short, full: c.full, date: c.date, subject: plain(c.subject), via: good[0].via, api: result.api, added, removed, files: [], sample: !full };
  good.filter((r) => full || r.id === 'dink-highearn').forEach((r) => {
    ...
    const wrapped = Object.assign({}, parsed, {
      lane2: { section: 5, commit: c.short, date: c.date, subject: plain(c.subject), via: r.via, specId: r.id, known: r.known, patchFallback: r.patchFallback || null }
    });
...
kept.forEach((k) => {
  L.push('## ' + k.date + (k.commit ? ' `' + k.commit + '`' : '') + ' ' + plain(k.subject));
  L.push('');
