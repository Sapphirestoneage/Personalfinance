...
(function (root, factory) {
  ...
  var RATIOS = [
    ...
    { id: 'withdrawalRate', gate: 'decumulation', label: 'Withdrawal rate', tier: 21,
      ...
      compute: function (c) {
        /* Still working: there is no rate to measure, because nothing is
           being drawn. Say where the question IS answered rather than
           stopping at "not for you" — the people who ask it are deciding
           whether they could stop, not reporting that they have. D-225. */
        if (!decumulates(c.household)) return Money.incomplete('Nothing is being drawn yet, so there is no rate to measure. To see what stopping would look like, run "Stopping, or coasting" in What If, Life.', []);
