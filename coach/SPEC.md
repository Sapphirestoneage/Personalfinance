...
**Keeps (as byte-identical copies, `coach/tools/vendor.js`):** the household model (`shared/schema.js`), the engines behind every figure it shows (Tier0, CashFlow, Opening, Coast, Debt, Countdown), the tables they read, and the vault for sealed files. Field ids are the SPARKS Ledger row ids. Nothing else of Money Rooms comes along.
...
**Does not change the public site.** A visitor to the public Money Rooms sees exactly what they see today. Coach Mode is its own app at `coach/index.html`: no Money Rooms page links to it, and nothing it stores is a Money Rooms key.
...
- **Left: the session path.** A checklist of stops (default below). Each stop expands into its items: the fields to fill, the questions to ask the client in plain words, and what "done" means. Checking an item ticks it for this client permanently. The current stop is highlighted.
- **Center: the work area.** The stop's own entry form: its fields as plain boxes (every one optional, each can be marked rough), its lists (debts, accounts, goals) one line each, and the read-outs the stop moves. No room is embedded (CD-003).
...
It parses into entries, shows a one-line preview ("Car loan: $450 a month, 5.9%, 38 payments left. Enter to save"), and saves on Enter. Anything it can't parse is saved as a note on the stop, never guessed into a number. Empty is not zero still holds.
...
Each stop in the JSON lists: its fields, the read-outs it shows, plain-words questions for Eli to ask, and its done-when test (evaluated, not typed).
...
- **Many households in one browser.** Coach Mode keeps its own store (`coach.` keys): a roster and one household per client. The personal household Eli keeps in Money Rooms is never read or written by it.
...
An **Import from sheet** step on Coach Home: Eli exports a client's Google Sheet tab as CSV, drops it in, and maps columns to fields once (the mapping is saved as a named template, so the second client takes seconds). Uses the SPARKS CSV reader (`shared/csv.js`, a vendored copy). Unmapped columns become notes on the client, not numbers.
