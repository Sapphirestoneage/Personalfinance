/* coach/console.js, the Session console (CD-006). See session.html. */
(function () {
  'use strict';
  ...
  function itemValue(it, household) {
    ...
    return (t || 'not yet') + (m && m.confidence === 'roughly' ? ', rough' : '');
  }
      ...
      if (isCur) {
        body = '<div class="stop-body"><ul class="items">' + s.items.map(function (it) {
          ...
          return '<li class="' + (itemAt[s.id] === it.id ? 'is-here' : '') + '"><label class="tick"><input type="checkbox" data-tick="' + esc(it.id) + '" aria-label="Done: ' + esc(itemLabel(it)) + '"' + (t ? ' checked' : '') + '/></label>'
            + '<button type="button" class="slaf-linkbtn item-text" data-item="' + esc(it.id) + '">' + (it.kind === 'question' ? 'Ask: ' : '') + esc(itemLabel(it)) + (v ? '<span class="coach-note">: ' + esc(v) + '</span>' : '') + '</button>'
  ...
  /* ---- The entry form (centre). Built once per stop: LIVE-FORM ---------------- */
  function boxFor(id, value, item) {
    var d = Fields.def(id) || {}, key = ' data-field="' + esc(id) + '"' + (item ? ' data-item-id="' + esc(item) + '"' : '') + ' aria-label="' + esc(d.label) + '"';
    var v = value;
    switch (d.unit) {
      case 'cents': return '<input type="text" inputmode="decimal"' + key + ' value="' + esc(v === null ? '' : String(v / 100)) + '"/>';
      case 'rate': return '<input type="text" inputmode="decimal"' + key + ' value="' + esc(v === null ? '' : String(Math.round(v * 1e6) / 1e4)) + '" placeholder="%"/>';
      case 'date': return '<input type="date"' + key + ' value="' + esc(v || '') + '"/>';
      case 'count': return '<input type="number" min="0" step="1"' + key + ' value="' + esc(v === null ? '' : String(v)) + '"/>';
      case 'bool': return '<select' + key + '><option value=""' + (v === null ? ' selected' : '') + '>Not said</option><option value="yes"' + (v === true ? ' selected' : '') + '>Yes</option><option value="no"' + (v === false ? ' selected' : '') + '>No</option></select>';
      case 'enum': return '<select' + key + '><option value="">Not said</option>' + (d.choices || []).map(function (c) { return '<option value="' + esc(c[0]) + '"' + (c[0] === v ? ' selected' : '') + '>' + esc(c[1]) + '</option>'; }).join('') + '</select>';
      case 'match': return '<span class="row"><input type="text" inputmode="decimal" data-match="pct" data-field="' + esc(id) + '" aria-label="Match, percent of what goes in" placeholder="match %" value="' + esc(v ? String(v.matchPercent * 100) : '') + '"/>'
        + '<input type="text" inputmode="decimal" data-match="cap" data-field="' + esc(id) + '" aria-label="Up to this percent of pay" placeholder="up to % of pay" value="' + esc(v ? String(v.matchCapPercentOfSalary * 100) : '') + '"/></span>';
      default: return '<input type="text"' + key + ' value="' + esc(v || '') + '"/>';
    }
  }
  function buildForm() {
    if (!current) { el('form').innerHTML = ''; return; }
    ...
    (current.fields || []).forEach(function (f) {
      ...
      parts.push('<div class="entry-row"><label class="inline">' + esc(d.label) + boxFor(f, Fields.read(household, f)) + '</label>'
        + '<label class="slaf-switch rough"><input type="checkbox" data-rough="' + esc(f) + '"' + (m && m.confidence === 'roughly' ? ' checked' : '') + '/> Rough</label></div>');
    });
    if (current.id === 'decisions') parts.push(decisionsHtml());
    el('form').innerHTML = parts.join('') || '<p class="coach-note">Nothing to type at this stop; talk it through.</p>';
    el('form-say').textContent = '';
  }
  function decisionsHtml() {
    var ds = rec().decisions;
    return '<fieldset class="entry-list"><legend>Decisions</legend>' + (ds.length ? ds.map(function (d) {
      return '<div class="entry-line"><label class="inline">Decision<input type="text" data-dec="label" data-dec-id="' + esc(d.id) + '" value="' + esc(d.label) + '"/></label>'
        + '<label class="inline">From (month)<input type="month" data-dec="startsOn" data-dec-id="' + esc(d.id) + '" value="' + esc(d.startsOn || '') + '"/></label>'
        + '<label class="inline">Verdict<select data-dec="verdict" data-dec-id="' + esc(d.id) + '">' + ['', 'go', 'wait', 'no'].map(function (x) { return '<option value="' + x + '"' + ((d.verdict || '') === x ? ' selected' : '') + '>' + (x ? x[0].toUpperCase() + x.slice(1) : 'No verdict') + '</option>'; }).join('') + '</select></label>'
        + '<label class="slaf-switch"><input type="checkbox" data-dec="showClient" data-dec-id="' + esc(d.id) + '"' + (d.showClient ? ' checked' : '') + '/> Show client</label></div>';
  ...
  function saveBox(t) {
    ...
    if (d.unit === 'match') {
      ...
      if ((a.value === null) !== (b.value === null)) { UI.say('form-say', 'The match needs both: how much, and up to what share of pay.', null); return; }
    ...
    UI.say('form-say', 'Saved: ' + d.label + (value === null ? ' (cleared)' : ''), 'good');
    paintLive();
  }

  /* ---- Read-outs and the rail ------------------------------------------------ */
  function line(cls, r, was) {
    return '<li class="' + cls + '"><span class="lbl">' + esc(r.label) + '</span><span class="now">' + esc(r.text) + '</span>'
      + (was !== null && was !== undefined ? '<br/><span class="was">at the start: ' + esc(was) + '</span>' : '')
      + (!r.ok && r.reason ? '<br/><span class="was">' + esc(r.reason) + '</span>' : '') + '</li>';
  }
  function paintFigures() {
    var now = h(), start = startHousehold();
    var ids = (current && current.figures) || [];
    var a = Session.figures(now, T, ids), b = start ? Session.figures(start, T, ids) : null;
    el('figures').innerHTML = a.map(function (r, i) { return line('zone-' + r.zone, r, b ? b[i].text : null); }).join('');
  }
  function paintRail() {
    var now = h(), start = startHousehold();
    var a = Session.rail(now, T), b = start ? Session.rail(start, T) : null;
    var fb = Session.fiBand(now, T), fbb = start ? Session.fiBand(start, T) : null;
    var fiNow = { label: 'FI date', ok: fb.hasDate, text: fb.hasDate ? 'likely ' + Session.monthsText(fb.likely) : fb.text, reason: null };
    el('rail').innerHTML = a.map(function (r, i) { return line('zone-' + r.zone, r, b ? b[i].text : null); }).join('')
      + line('', fiNow, fbb ? (fbb.hasDate ? 'likely ' + Session.monthsText(fbb.likely) : fbb.text) : null)
      + (fb.hasDate ? '<li><span class="was">' + esc(Session.monthsText(fb.best) + ' to ' + Session.monthsText(fb.worst)) + '</span></li>' : '');
  }
  ...
  /* ---- Top bar -------------------------------------------------------------- */
  function paintTop() { var s = open(); el('btn-start').hidden = !!s; el('btn-end').hidden = !s; if (!s) el('timer').textContent = 'not started'; }
  ...
  function paintLive() { paintPath(); paintFigures(); paintRail(); paintNotes(); }
  function paintAll() {
    refreshClient();
    stops = Session.path(T.sessionPaths, client.pathId, client);
    if (current) current = stopById(current.id);
    paintTop(); paintPath(); paintCrumbs(); paintFigures(); paintRail(); paintNotes(); paintCommentTargets();
  }
  function show(stopId, opts) {
    ...
    if (moved || (opts && opts.rebuild)) buildForm();
    paintAll();
  }
  ...
  el('quick-in').addEventListener('keydown', function (e) {
    ...
    try {
      ...
      this.value = '';
      buildForm();                      /* the coach is typing up here, not in the form */
      paintLive();
  ...
  document.addEventListener('click', function (e) {
    ...
    if ((b = t.closest('[data-add]'))) { Coach.addItem(ID, b.getAttribute('data-add'), {}); buildForm(); paintLive(); return; }
    if ((b = t.closest('[data-remove]'))) { var pr = b.getAttribute('data-remove').split(':'); Coach.removeItem(ID, pr[0], pr[1]); buildForm(); paintLive(); return; }
    ...
    if ((b = t.closest('[data-add-note]'))) {
      ...
      var boxN = el('note-' + scope + '-' + kind), s0 = open();
      if (!boxN.value.trim()) return;
      if (scope === 'session' && !s0) { UI.say('top-say', 'Start the session first; a session note belongs to one.', 'bad'); return; }
      Coach.addNote(ID, { kind: kind, text: boxN.value, stopId: scope === 'stop' && current ? current.id : null, sessionId: s0 ? s0.id : null });
      boxN.value = ''; paintLive(); return;
    }
    ...
    switch (t.id) {
      ...
      case 'btn-dec-add': Coach.addDecision(ID, {}); buildForm(); paintLive(); return;
      case 'btn-start': {
        Coach.startSession(ID); visited = {}; if (current) visited[current.id] = true;
        UI.say('top-say', 'Session started. The numbers as they stand are frozen for the recap.', 'good'); paintAll(); return;
      }
      case 'btn-end': {
      ...
      case 'btn-recap-seal': {
        ...
        Coach.saveRecap(ID, recapFor, el('recap-text').value);
        Coach.seal(Coach.recapFile(ID, el('recap-text').value), pass).then(function (sealed) { UI.download(Coach.filename('recap'), sealed); UI.say('recap-say', 'Saved a sealed recap. Send the passphrase to the client another way.', 'good'); },
