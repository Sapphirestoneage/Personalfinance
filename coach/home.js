/* coach/home.js, Coach Home (D-338, D-341). See the comment in index.html. */
(function () {
  'use strict';
  ...
  var Profiles = SLAF.Profiles, Reference = SLAF.Reference, LedgerRows = SLAF.LedgerRows;
  var T = null;
  var importing = null;              /* { text, headers } of the file read for import */

  if (!UI.guard(el('main'))) return;
  /* Home is where the tab goes back to the coach's own household: a client
     is switched to only by opening their session or view from here. */
  Spine.useProfile('default');

  function blocksOf(id) {
    var raw = Profiles.readKeys(id)['slaf.scenarios.v1'];
    try { var m = raw ? JSON.parse(raw) : null; return m && Array.isArray(m.blocks) ? m.blocks : []; } catch (e) { return []; }
  }
  function summary(c) {
    var h = Spine.householdOf(c.id) || Schema.createHousehold({});
    var rec = Schema.createCoachRecord(h.coach || {});
    var stops = Session.path(T.sessionPaths, c.pathId, c);
    var st = Session.stage(stops, h, T, { notes: rec.notes, homework: rec.homework, blocks: blocksOf(c.id), verdicts: c.blockVerdicts || {} });
    ...
    var first = sessions.length ? sessions[0] : null;
    var bandNow = Session.fiBand(h, T);
    var bandFirst = null;
    if (first && first.startSnapshotId) { var fh = Coach.snapshotHousehold(first.startSnapshotId, c.id); if (fh) bandFirst = Session.fiBand(fh, T); }
    var rough = null;
    try { rough = Session.roughCount(LedgerRows.summary(h, T)); } catch (e) { rough = null; }
    return { h: h, rec: rec, stops: stops, stage: st, sessions: sessions, last: last, bandNow: bandNow, bandFirst: bandFirst, rough: rough, checkin: Session.checkinStatus(rec.checkins) };
  }
  function bandShort(b) {
    if (!b) return 'no session yet';
    if (!b.hasDate) return b.text;
    return 'likely ' + Session.monthsText(b.likely);
  }
  function row(c) {
    var s = summary(c);
    ...
    return '<tr data-client="' + esc(c.id) + '">'
      ...
      + '<td data-label="Rough or stale">' + (s.rough === null ? 'not yet' : esc(String(s.rough))) + '</td>'
      + '<td data-label="Check-in"><span class="status ' + esc(s.checkin) + '">' + esc(checkWord) + '</span></td>'
  ...
  function paint() {
    var archived = el('show-archived').checked;
    var list = Coach.clients().concat(archived ? Coach.clients({ archived: true }) : []);
    ...
    var tkeep = el('imp-template').value;
    var ts = Coach.templates();
  ...
  /* ---- History (E4): every session, its recap, its snapshot read-only ---- */
  function showHistory(id) {
    ...
    el('history').innerHTML = s.sessions.slice().reverse().map(function (x) {
      ...
      return '<li><b>' + esc(UI.day(x.startedAt)) + '</b> ' + (x.endedAt ? esc(UI.duration(x.durationMs)) : '(still open)')
        + (covered ? ', ' + esc(covered) : '')
        + (snap ? ' <a href="client.html?client=' + encodeURIComponent(id) + '&amp;snap=' + encodeURIComponent(snap) + '">The plan as of that day (read-only)</a>' : '')
        + (x.recapText ? '<details><summary>Recap</summary><pre style="white-space:pre-wrap">' + esc(x.recapText) + '</pre></details>' : '')
        + '</li>';
  ...
  function passphrase() {
    var p = el('pass').value;
    var st = SLAF.Vault.strength(p);
    if (!st.ok) { UI.say('backup-say', st.why, 'bad'); el('pass').focus(); return null; }
    return p;
  }
  function backup(obj, kind) {
    var p = passphrase(); if (!p) return;
    UI.say('backup-say', 'Sealing...');
    Coach.seal(obj, p).then(function (text) {
      UI.download(Coach.filename(kind), text);
      UI.say('backup-say', 'Saved a sealed file. It opens only with that passphrase.', 'good');
    }, function (e) { UI.say('backup-say', e.message, 'bad'); });
  }

  /* ---- Sheet import (B3) -------------------------------------------------- */
  function targetOptions(chosen) {
    return '<option value="">Keep as a note</option>' + Coach.sheetTargets(T).map(function (t) {
      return '<option value="' + esc(t.id) + '"' + (t.id === chosen ? ' selected' : '') + '>' + esc(t.label) + '</option>';
    }).join('');
  }
  ...
  document.addEventListener('click', function (e) {
    ...
    if (t.id === 'btn-new') {
      try {
        var nc = Coach.addClient({ name: el('new-name').value, pathId: el('new-path').value });
        el('new-name').value = '';
        paint();
        UI.say('new-say', nc.name + ' was added. Open session starts on the path.', 'good');
    ...
    if (t.id === 'btn-restore') {
      ...
      UI.readFile(el('restore-file')).then(function (text) { return Coach.openFile(text, p); }).then(function (file) {
        ...
        var no = res.filter(function (r) { return !r.ok; }).map(function (r) { return r.reason; });
        paint();
    ...
    if (t.id === 'btn-imp-go') {
      ...
      var cols = columnsNow();
      var plan = Coach.sheetPlan(importing.text, cols, T);
      var name = el('imp-name').value.trim();
      if (name) Coach.saveTemplate(name, cols);
      /* The one write into a client from this page: switch, write through
         Ownership, switch straight back. Nothing is drawn in between. */
      var out;
      try { Spine.useProfile(id); out = Coach.applySheet(plan, T); }
      catch (err) { UI.say('imp-say', err.message, 'bad'); }
      finally { Spine.useProfile('default'); }
      if (out) UI.say('imp-say', out.written + ' numbers written to ' + Coach.client(id).name + '; ' + out.notes + ' kept as notes.' + (plan.bad.length ? ' Not read as numbers: ' + plan.bad.join(', ') + '.' : ''), 'good');
      paint();
      return;
    }
  });
