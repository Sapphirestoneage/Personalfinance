...
This is a standalone tool, but it stores your sheet in the exact shape the
larger [SPARKS / SLAF](https://github.com/Sapphirestoneage/Personalfinance)
suite uses — a real household object, not a private bag of fields.
...
original namespace so a diff against the source repo is exact. The single
exception is the table list in `shared/reference.js`, which had to be trimmed
to the tables this product actually ships — it is commented as such.

## Running it

No build step, no dependencies, no framework. It does need to be *served*
rather than opened as a file, because it fetches its data:
