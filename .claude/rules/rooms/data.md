---
paths:
  - "rooms/data.html"
  - "engines/bankcsv.js"
---
# Your Data (`data`)
File: rooms/data.html · 824 lines · utility room
Engines: tax, subscriptions, merchants, bankcsv
Reference data: confidence_weights.json, effective_tax_rates_2026.json, expense_categories.json, import_keywords.json, ledger-rows.json, staleness.json, +2 more
Owns: nothing
Reads from other owners: nothing traced
Latest decisions:
  - D-306 — A statement in, every place money went, and the slope of a month
  - D-305 — One door in, one door out, and a backup nobody else can open
  - D-221 — The CSV round trip, made to survive a real spreadsheet
  - D-220 — One CSV out, and the same CSV back in
  - D-125 — Your Data: a file added or replacing, and a pasted statement sorted
Full context: node tools/context/pack.js data
