---
paths:
  - "rooms/statements.html"
  - "engines/statements.js"
---
# Your Statements (`statements`)
File: rooms/statements.html · 385 lines
Engines: projection, tier0, cashflow, statements
Reference data: expense_categories.json
Owns: nothing
Reads from other owners: grossAnnualIncome (income), cashSavings (start), employerMatch (start), contributionPercent (start), totalDebt (debt-payoff), monthlyDebtPayments (debt-payoff), otherAssets (statement), netWorth (statement), +1 more
Latest decisions:
  - D-156 — Your Statements: the three documents, and the basis printed on them
Full context: node tools/context/pack.js statements
