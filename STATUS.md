# STATUS (keep under 40 lines; /wrap rewrites it)

Updated: 2026-09-19

## Where it stands
- **93 → 32 rooms**, 62 redirect stubs, nothing deleted; `docs/room-map.json`
  is the cut list, checked every run. What If earns a room (D-296); Start
  Here waits on an owner decision (item 1).
- **The book, reconciled (D-297..D-301)**: the city index is read where it
  is owned; the ratio glossary was already Every Ratio and now answers to
  its names; a Decision Room block can PAY you; eight templates, five
  reversibility rows and two debates landed as rows; Tax has a second
  reading, The cliff. No new room. `ROADMAP.md`'s not-built list is honest
  again: six, each blocked on a dataset or an instruction.
- **Main merged in (D-302)**: its D-234..D-263 land in the rooms that
  absorbed theirs; this lane renumbered to D-264..D-301. Another lane
  works on main directly, so merge it before every push.
- **Three merged sessions from the phone walk (D-303, D-304, D-305).** The
  screens as a person on a phone meets them: the math sheet opaque over a
  dimmed page, a gutter on the intake rooms, switches that look on or off, a
  Save that gives a spreadsheet or a page and names the JSON as the restore
  file (D-303). Express made short: each level renders only its own fields,
  a level badges Done or a count, the walk steps one level a screen, a line
  has a name, an institution and a last four, and lines group under their
  bank; it lives in the Ledger's all-at-once door (D-304). Your Data is
  three cards with one door in, and a backup can be sealed with a
  passphrase on the browser's own crypto, with a fingerprint after a save
  and a load (D-305).
- **The reframe D-228 asks for has NOT shipped**; the FI target is still
  spending ÷ the withdrawal rate, and both rooms say so in their spec.
- **The Calendar is back (D-306)**, at the owner's call: the month picture
  The Month draws, plus your own dates (a card to apply for, a renewal to
  cancel, a form due), drawn on the month and never counted. Thirty-two
  rooms. All three intakes stay (D-307).
- **Freeze ON, and honoured** for everything but the one room the owner
  asked back.

## Next (top item first; one per session)
1. ~~Which intake survives?~~ Decided (D-307): all three stay. Round 1 and
   All at once in the Ledger, Start Here as its own room.
2. **OWNER DECISION: is the income floor means-tested?** (D-228, D-284.)
   The FI target waits on it, and it moves the number every room shows.
3. **OWNER DECISION: the Trust Framework** — three vetting questions,
   shippable as they stand, but a lens under the freeze. Yes or no.
4. `data/benefit_cliffs_2026.json` is `unverified`; the three lines want a source.
5. Walk the rooms off the path at 390px the way D-303 walked six: a floating
   panel that is not opaque, a wrapper with no gutter, inline spans where
   lines were meant, a control with no on state. A test that catches them
   beats a walk that finds them.

## Known open
- Nothing. Every CI step is clean on this tree, browser gates included.

## Panel notes
- Galloway: the means-tested floor, item 2. Hormozi: thirty-two is still a
  library — one screen in front of it. Donegan: a block that pays is the
  first thing here about building income, not just having a job.
