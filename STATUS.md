...
- **Main is worked on by another lane**: merge before every push. D-228's reframe has NOT shipped. Freeze holds (D-313).
  Coach Mode is its own lane in `coach/` (D-339, `coach/STATUS.md`); Parnassah, the
  Modern Orthodox planning site, is its own lane in `parnassah/` (D-340, `parnassah/STATUS.md`).
