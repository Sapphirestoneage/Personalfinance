...
- **Facts are entered in the Ledger only (D-313)**, one owner per row. New rooms (D-316):
  ...
  Cushion's (D-314), pay to come (D-315). On the cards (D-337, from main).
- **The Solar System (D-320 to D-330)**: `docs/SOLAR-SYSTEM.md` is the spec; 180 levels,
  ...
  card.
- **Anyone can answer it (D-336, D-338)**: band 1 in plain words, with an add-it-up fold
...
- **The Planets dashboard (D-339)**: an Overview tab: a ring, four tiles, bars by planet
  and band, readings by tier, each against its normal range.
- **The standard (D-340)**: `docs/DESIGN.md`, 23 rules each with its check. No unrounded
  number reaches a screen, undo docks in the page, one row of tabs, small print folded.
- **Nothing moves under a finger (D-341)**: the toast left the bottom of the screen (it
...
- **The measuring stick draws (D-342)**: DRAFTT is seven bullet bars on one axis, the band
  a stripe behind each, the figure and the verdict in words beside it; whose bands is
  asked once; a bar label is never cut off on a phone.
