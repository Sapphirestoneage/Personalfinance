...
| **T8** — FI-losophy rooms | parked | Designed Week, Enough, Unlearning, Time Buckets, Dreamline, time-denominated display — set aside for the one-pager pass (D-094); the foundations sit in a stash and the list is in `LATER.md` |
| **One pager in, one pager out** | **in progress** | The core: the spine's command log (undo / redo on every page), the gate and `Gate.exists`, the lens, `Registry.REQUIRES` (D-094) · the one-pager: one gate, ten cards at most, guesses with badges, the fine-tune drawer, paste import (D-095) · the dashboard as home: four blocks, the situation's lead first, the Advice Translator, the date with the lens (D-096) · the room template, `shared/room.js`, proven on Real Hourly Wage and frozen, with Get Help where every scope line points (D-097) · twelve rooms on it: Between Jobs, Protection, Decumulation, Tax, Estate Basics, Giving, Career Move, Partner, Kids and Tuition, Housing Decision, Big Purchase, Variable Income (D-098–D-113) · the LATER.md items that are not rooms (D-100) · next: the LATER.md rooms and History (D-101, D-114 onward) |
