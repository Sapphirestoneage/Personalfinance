...
Twenty-seven rooms are live. Against this document's own tiers:
...
| **3 and up** | ~280 | **3** | Regret calc (Tier 4) — not a build of its own: it is `Worth.regrets()`, the same records filtered to low ratings. **DND Character Sheet** (Tier 5) and its Bestiary — the first Tier 5 framework built, and the first room outside `SPEC.md`'s Tier 0–2 scope to get its own engine. Everything else is out of `SPEC.md` scope |
...
The first **Tier 5 framework** is now built, which is a departure worth
naming: everything shipped before it lives in `SPEC.md`'s Tier 0–2 scope. The
**DND Character Sheet** implements the Dungeons & Dividends rulebook — six
stats over eighteen sub-stats, HP measured in weeks of runway, and a level
that is a percentage of *your own* FI number rather than a dollar amount, so
a lean and a fat character both reach 20 at 100% of a different number. It
adds no figure the household did not already hold: nine of the eighteen
sub-stats read from income, expenses, balances and debts through `Tier0`, and
the other nine are generated by whichever of five methods you pick, the way a
table picks ability scores (D-047). The **Bestiary** is its reference half —
Monster Manual, Hazards, the Revenue Guild, Status Effects and the three Feat
Trees — and reads nothing at all.

The rulebook contradicted itself twice and both are recorded rather than
quietly resolved: HP was defined both as months of runway and as a Hit Die
formula, which is settled at 1 HP = one week (D-046); and the class count is
seven, not the ten one later sentence implies (D-048).

Still unbuilt inside that room, each noted in the code where it would attach:
the Multiclass Gates of §5A, live encounters against the Monster Manual,
quest tracking, and three of the five result formats in §12A. The Spectrum is
built, because it is a pure reading of the eighteen.
...
- [x] ✅ DND Character Sheet — **BUILT** (`rooms/character-sheet.html` + `rooms/dnd-reference.html`, D-046/047/048)
