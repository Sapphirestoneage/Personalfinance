...
| **Every room fits your situation** | **shipped** | `Gate.exists` had known which rooms belong to which situation since D-094 and only the map and the menu ever asked — so someone between jobs opened Real Hourly Wage and was asked for their commute, and thirteen more rooms asked for contract rates, a 401(k), a partner's income, a child's tuition. Fixed in the one function every room already reaches: the room folds behind a notice naming the situation, giving the reason in a plain sentence, pointing at what does apply, and offering "Show it anyway". The gate gained a sentence per branch so a room never hides in silence. 342 room-situation pairs measured: 42 wrong before, 0 after. D-142 |
| **The panel audit** | **shipped** | Four reviewers on the dashboard's full panel — plain language, information architecture, partial data, layout and accessibility — each measuring in a browser. **The crash**: typing 0 for a month of spending blanked every panel and blamed `data/`, through four frames that each passed a non-Result along. **The untruths**: "every banded ratio sits at or inside the comfortable edge" (inside is the bad side, seven ambers uncounted, 22 bands claimed); a green tint painted on the bad region; an unfunded goal reading `$0 of $30,000 · 0%` exactly like one told it was zero; an ⓘ saying three over a panel saying two; a 404 to The Score. **The gate**, again: a retiree shown their FI ratio in red on an accumulation ladder — eleven ratios now carry the branch they belong to and the radar takes a predicate. D-143. **The measurements**: 15 AA contrast failures inside the panel → 0, ten of them one token at 2.63:1; the drawer had no disclosure triangle at all on touch; the ⓘ toggles were 16.5px; the undo pair covered the one out-of-range figure; the page told the browser it was light. D-144 |
| **The Skill Tree & the Exercise Library** | **shipped on a seed** | `engines/skilltree.js` and `rooms/skill-tree.html`: five bands, the FOO ladder as the fortress line above them, four states plus fog, every lock saying why, boosts that open and never award, warps that reveal and never award; `engines/exercises.js` and `rooms/exercises.html`: five kinds, the twelve runs computed through the owning engines, the thirteen canon exercises credited; the Stacker and dashboard block 3 read the tree; `version.json` and the footer. Ran on a 40-skill seed until the real curriculum arrived. D-131 · **The curriculum, landed**: `scripts/extract-v63.mjs` reads FI-Skill-Tree-v6.3.1 for real — 25 trees × 25 levels, five bands, 312 cross-tree shortcut lanes — and merges the app's own 40 skills, preserved in `scripts/skill_tree_app.json`, so every id the exercises, the FOO links and the Stacker point at still resolves. **31 trees, 665 skills, 312 lanes.** The board's cards were resized and its state badge moved so a name at that density stays inside its box, measured in a browser by `test/alignment.js`. D-139 · **The board, redrawn as a tech tree**: rows are trees and columns are rungs, five columns to a band, so each of the 31 trees reads left to right rung by rung. The tree names sit in a rail that does not scroll; the 312 shortcut lanes are drawn faint under the rungs and light up when you pick a node; the room takes the whole window and the prose keeps the measure beside it. 1,200 × 13,184px became 6,000 × 2,838. D-140 · **The card says what the curriculum says**: what it is, what it does for you, where it fits and the tier were in the data and never reached the screen; all four now show, absent lines are dropped rather than printed empty, and the fog covers them like the name. D-141 |
...
| **Debt Payoff: one debt, one screen** | **shipped** | A single debt filled a phone screen, so a household with three of them could not be read. Four facts stay up — balance, minimum, type, how interest works — and everything set-once folds behind a caret: *Why you're keeping it* (the feeling and the reasons) and *Dates & limit*. Each closed drawer carries a live summary, so nothing hides silently, and open/closed survives a rebuild so a drawer cannot snap shut under a finger. The room widens past the shared 480px measure, because it is an editor rather than something you read. D-134 |
| **The menu** | **shipped** | Every page gets a menu button that opens a panel listing *Your data & upkeep* first — Your Data, Refresh, History, Start Here, Get Help, the map — then every room by kind, with the current one marked. Closes on ✕, backdrop or Escape. At 1080px and wider it pins open as a sidebar and the page sits beside it. Mounted from the one `.room-back` link every page already has, so no room carries markup for it. The desktop column grows from 480px to 620/680px. D-135 |
| **Phone and desktop, measured** | **shipped** | Every room audited at 412px and 1440px. Fixed: the 69px sideways scroll on Every Ratio (a reason in a figure's cell, nowrapped) · touch targets under a coarse-pointer query — the suggestion chip, ticks at 20px, summaries, the exercise chips, the ⓘ at 30px — while a mouse keeps the compact controls · the measure stepping to 720px and stopping, because a wider paragraph is not a better one · the map flowing two-up. Inline links in prose left small on purpose. Per-room desktop layouts deliberately not guessed at. D-136 |
...
| **T14** — The whole run through | **shipped** | Five questions → Your six → Your character → Getting better → Play, all on one page. The quiz reads temperament (which lever you reach for) and is scored as a share of each class's own ceiling so it is not rigged — a test walks all 2,000 answer sets and insists every class wins for somebody and none takes over 25%. Where the quiz class and the measured class differ, the page says so: you play like one lever and your money moves through another. Four build routes (real numbers, point buy, standard array, roll), 54 improvement moves across all 18 sub-stats ranked weakest-first, and a focused game that deals situations training the ability you picked — focus is a preference, not a filter, so a chapter never repeats. 24 new scenarios, mostly about earning, because Strength previously had one. Acronyms gone from every label a player reads (DD-026) |
| **T15** — The long read | **shipped** | `profile.html` + `engines/persona.js` + `data/dnd_profile.json`: an Enneagram-shaped profile per class — core desire and fear, the lie and the truth, at best and at worst, the blind spot, where you grow and where you slide under stress, five named relationships, and the creature you are weak to (with its real CR and save). "Who finds you annoying" is read from everyone else's profile, so it is their opinion rather than your guess. Where the quiz class and the measured class disagree it shows both — "there are two of you". The childhood section is **offered, never asserted**: three doors and a None of these, with a warning printed above them that the tool cannot see a childhood and would be making it up, enforced by four tests. Plus an almanac — sign, glyph, element, ruling body, hour, metal, stone, tarot card, alignment, patron and pact, curse, resistances, bane, omens and a horoscope reading, all invented and labelled as such — and a weakness list that names three kinds of PERSON and three CIRCUMSTANCES alongside the creature, each with the mechanism behind it. Plus a big three — first (where your money moves), second (what you fall back on), third (what people meet first), each reading differently for the same class — and the SHADOW, the lever at the bottom you never pull, with what to be on the lookout for (DD-027) |
| **T16** — Four ways in | **shipped** | A design panel found the same fault five ways: the tool asked before it gave, and three of its four build routes could not show a finished character at all. Three new doors: **the Descent** (`descent.html`) — one number buys one true sentence, $2,500 a month tells you your $750,000, every rung built at boot so the phone keyboard survives; **the Starting Six** (`data/dnd_pregens.json`) — four complete characters playable in one tap, loaded through the ordinary import path, badged as borrowed everywhere they show, and tested on the lesson rather than the file; **the Menagerie** (`menagerie.html`) — 29 creatures as the front door, "this one got me" as input, labelled `recognised` and never written to a score. Plus a front page that offers all four and says what each costs. The Fellowship and Table Mode are specced, unbuilt (DD-028) |
| **T17** — A sheet that travels | **shipped** | `card.html` draws the 5e silhouette everyone recognises — six ability boxes with the modifier in a circle, a shield for Armour Class, hit points captioned *weeks of runway*, a Level box, saving throws with proficiency dots, a features panel with the class headline, lever, condition and what hunts you — in canvas, two skins, from the same engines as every other page. The sheet **never carries a typed figure** (a test slices its data function and checks), bought scores draw dashed, and an unmeasured slot is captioned rather than dashed. Six pages carry Open Graph / Twitter tags so a pasted link unfurls as a picture, `og.png`, rendered from a ready-made character by the real renderer. Share moments on the character screen and every chapter review; the last engine names (*the ladder*, *sub-stat*) gone from on-screen copy (DD-029) |
| **The FIRE Lab** | **shipped** | Every FIRE calculation on one screen and drawn instead of listed: the number beside a progress ring, a withdrawal-rate control that moves the whole page and stores nothing, the six flavours as bars, a donut of where the number comes from, milestones at 25/50/75/100%, the portfolio path against the line it is aiming at, and a four-by-five sensitivity grid with your square lit. Owns no formula and writes no field. `rooms/fire-lab.html`. D-138 |
