...
| **T5** — The Rerank | **shipped** | `rooms/rerank.html` + `engines/rerank.js` + `data/common_costs.json`: four stages, cost order against value order, cut / keep / ok with the max(3, ¼) threshold, the year flagged and its 25× (D-084) |
| **T6** — Life events + Triple D | next | One engine, one room, templates as data |
