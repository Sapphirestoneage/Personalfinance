...
| **T6** — Life events + Triple D | **shipped** | `engines/events.js` + `data/triple_d.json` + `data/return_bands.json`, `rooms/what-if-life.html`, the template schema, and the first template, the sabbatical (D-086) · templates so far: kids, job-change, house, freelance, move, debt-sprint, big-purchase, retire-or-coast with `engines/vpw.js` and `engines/ss.js`, partner-merge (D-087) — all ten · the dashboard's 3D toggle (D-088) |
| **T7** — Skill Stacker | next | New room, reads the `dnd/` catalogue |
