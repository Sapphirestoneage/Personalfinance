...
| **T6** — Life events + Triple D | in progress | `engines/events.js` + `data/triple_d.json` + `data/return_bands.json`, `rooms/what-if-life.html`, the template schema, and the first template, the sabbatical (D-086) · templates so far: kids, job-change, house, freelance, move, debt-sprint (D-087) |
