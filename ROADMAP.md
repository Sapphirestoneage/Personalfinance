...
| **One pager in, one pager out** | **in progress** | The core: the spine's command log (undo / redo on every page), the gate and `Gate.exists`, the lens, `Registry.REQUIRES` (D-094) · next: the input page, the dashboard, the room template on Real Hourly Wage, then the rooms in tranches, History last |
