...
const CASES = [
  ...
  },
  {
    ...
    container: '#xform',
    seed: 'empty',
    /* A person who has answered nothing now lands in the guided walk (D-303),
       which shows one family at a time. This case is about the flat form, so
       it asks for the flat form first. */
    prepare: async (page) => {
      await page.waitForSelector('[data-mode="all"]');
      await page.tap('[data-mode="all"]');
      /* D-304: every level is a fold now, and only the first unfinished one
      ...
      /* The situation is six choices, which is a select now, not six buttons
         (D-304): five choices as five buttons was two rows of chrome for one
    ...
    fields: [
      ...
      /* D-304: a card is a name, a lender and a last four, three fields, so
  ...
  {
    /* EXPRESS, THE GUIDED WALK (D-303): the same rows, one DAITE family a
