...
(async () => {
  ...
  /* A preference row (the ask, D-317; Coach Mode, D-338) is a switch for the
     person, not a feature a room shows: it has no rooms to name. */
  const featureRows = rows.filter(r => ids.indexOf(r.id) !== -1);
  check('every row names where it shows up', featureRows.every(r => r.links >= 1), featureRows.filter(r => r.links < 1).map(r => r.id).join(','));
  ...
  check('every user switch is on', (await page.$$eval('button[role="switch"][data-feature]', bs => bs.every(b => b.getAttribute('aria-checked') === 'true'))));
  for (const id of userIds) { await page.tap('button[data-feature="' + id + '"]'); await page.waitForTimeout(80); }
  check('...then every user switch is off', (await page.$$eval('button[role="switch"][data-feature]', bs => bs.every(b => b.getAttribute('aria-checked') === 'false'))));
  ...
  check('Beginner: only the default-on set', (await page.$$eval('button[role="switch"][data-feature]', bs => bs.map(b => b.getAttribute('data-feature') + ':' + b.getAttribute('aria-checked')).sort().join(','))) === userIds.map(id => id + ':' + (table.features[id].default === 'on')).sort().join(','));
  await page.tap('button[data-path="fi"]'); await page.waitForTimeout(150);
  check('FI: Accuracy and Horizon all on', (await page.$$eval('button[role="switch"][data-feature]', bs => bs.map(b => b.getAttribute('data-feature') + ':' + b.getAttribute('aria-checked')).sort().join(','))) === userIds.map(id => id + ':' + (['accuracy', 'horizon'].indexOf(table.features[id].group) > -1 || table.features[id].default === 'on')).sort().join(','));
