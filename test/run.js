...
const TABLES = {
  ...
  irsLimits: require(path.join(ROOT, 'data/irs_limits_2026.json')),
  dndRules: require(path.join(ROOT, 'data/dnd_rules.json')),
  dndClasses: require(path.join(ROOT, 'data/dnd_classes.json')),
  dndScoring: require(path.join(ROOT, 'data/dnd_scoring.json'))
};
...
/* ==========================================================================
   Dungeons & Dividends — the character sheet's scoring engine.
   The rulebook left every dollar-to-score threshold unspecified (its own
   Section 13 says so), so these checks pin down the calibration that was
   agreed instead: a US-population-median household scores 10 on every
   computed sub-stat. Change data/dnd_scoring.json and this section is what
   tells you which promises you just broke.
   ========================================================================== */
...
section('Dungeons & Dividends — the demo persona, end to end');

(function () {
  const s = Character.sheet(h, TABLES);

  /* Robin carries one card at 22.9% and a student loan at 5.5%. The
     high-interest line is foo_rules.json's, reused rather than redefined, so
     this room and the FOO room can never disagree about what "high" means. */
  check('the high-interest threshold comes from foo_rules',
    s.debtBurden.highInterestRate, TABLES.fooRules.thresholds.highInterestDebtRate);
  check('one high-interest balance puts Robin at Debt Burden 2', s.debtBurden.value, 2);

  /* A $72k paycheque outranks $48k invested and $9.5k cash. */
  check('Robin is sorted into The Earner', s.suggestedClass.value, 'earner');
  const levers = Character.leverActivity(h, TABLES);
  checkTrue('The Keeper is scored on restraint, not on a pool it does not own',
    Money.isEntered(levers.keeper));

  /* Level is Tier0.fireProgress read against the band table — never a second
     FIRE calculation. If the two ever disagree, this fails. */
  const prog2 = Tier0.fireProgress(h, TABLES);
  check('level agrees with Tier0 fireProgress',
    s.level.value, Character.levelFromProgress(prog2.value, TABLES).value);

  /* CON cannot score: the persona has never answered the consistency input.
     So Max HP is withheld rather than assumed — the whole point. */
  checkTrue('CON is unscored without its third sub-stat', !Money.isOk(s.stats.CON));
  check('so Max HP is withheld', s.maxHp, null);

  /* But current HP is measurable from balances alone, in weeks. */
  checkTrue('current HP still reads from real balances', Money.isOk(s.currentHp));
  const weekly = Character.weeklyExpensesCents(h);
  const liquid = Character.liquidAssetsCents(h);
  check('current HP is liquid assets over one week of expenses',
    s.currentHp.value, Math.floor(liquid.value / weekly.value));
})();
