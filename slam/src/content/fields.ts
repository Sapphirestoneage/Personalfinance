...
export const SHARED_FIELDS: FieldMeta[] = [
  ...
  f('hourlyValueCents', 'dollars', 'What an hour of yours is worth', 'Hourly value', 'Hourly value (opportunity cost)', 'Used to price the time a move costs.'),
  f('acquisitionSpendCents', 'dollars', 'Money spent finding clients a month', 'Acquisition spend a month', 'Acquisition spend / month', 'Ads, listings, promo.'),
  f('acquisitionHoursPerMonth', 'hours', 'Hours spent finding clients a month', 'Acquisition hours a month', 'Acquisition hours / month'),
];
...
export const BUSINESS_FIELDS: Record<BusinessType, FieldMeta[]> = {
  inPerson: [
    ...
    f('spaceCostCents', 'dollars', 'Space cost a month', 'Space cost a month', 'Space cost / month (fixed)', 'Leave at 0 if it is already inside fixed costs.'),
  ],
  content: [
    ...
    f('ppvSalesPerMonth', 'count', 'Pay-per-view sales a month', 'PPV sales a month', 'PPV units / month'),
    f('customsPerMonth', 'count', 'Customs a month', 'Customs a month', 'Custom orders / month'),
    f('digitalSalesPerMonth', 'count', 'Digital product sales a month', 'Digital sales a month', 'Digital product units / month'),
    f('hoursPerMonth', 'hours', 'Hours a month on content', 'Content hours a month', 'Content hours / month'),
    f('toolsCostCents', 'dollars', 'Tools and apps a month', 'Tools cost a month', 'Tools cost / month (fixed)'),
  ],
  calls: [
    ...
    f('depositCoversNoShow', 'flag', 'Paid even when they do not show', 'Deposit covers no-shows', 'Deposit forfeits on no-show'),
  ...
  regulars: [
    ...
    f('oneOffTributeAvgCents', 'dollars', 'Typical one-off tribute', 'Average one-off tribute', 'Average one-off tribute'),
    f('chargebackRate', 'percent', 'Share lost to chargebacks', 'Chargeback rate', 'Chargeback rate'),
...
const fee = f('feeRate', 'percent', 'Cut the platform or processor takes', 'Platform cut', 'Fee rate');
...
export const OFFER_FIELDS: Record<OfferType, FieldMeta[]> = {
  single: [price, f('minutes', 'minutes', 'Minutes', 'Minutes', 'Duration (min)'), fee, variable, allIn],
  ...
  call: [f('priceCents', 'dollars', 'Price per block', 'Price per block', 'Price / block'), f('minutes', 'minutes', 'Minutes per block', 'Block length', 'Block (min)'), fee, variable, allIn],
  tribute: [
